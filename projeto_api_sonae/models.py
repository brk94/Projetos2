...
# Alias literal para cargos (útil em validação Pydantic)
...
# ======================================================================================
# ORM: PROJETOS / RELATÓRIOS
# ======================================================================================
class Projeto(Base):
    __tablename__ = "projetos"

    # Identidade e metadados do projeto
    codigo_projeto   = Column(String(50), primary_key=True, index=True)   # ex.: "PROJ-001"
    nome_projeto     = Column(String(255), nullable=False)
    gerente_projeto  = Column(String(255))  # nome textual; FK opcional abaixo
...
class Usuario(Base):
    ...
    senha_hash       = Column(String(255), nullable=False)  # bcrypt ou legado (texto)
    data_criacao     = Column(DateTime, default=datetime.utcnow)
    data_atualizacao = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class RefreshToken(Base):
    ...
    token_hash    = Column(String(255), nullable=False)  # **apenas** hash
...
class UsuarioSolicitacaoAcesso(Base):
    ...
    # Status com domínio fixo
    status = Column(Enum('aguardando', 'aprovado', 'rejeitado', 'expirado', name='status_solic'), nullable=False, default='aguardando')

    # Decisão administrativa (FK lógica ao usuário decisor)
    ...
    # Coluna gerada (MySQL 8+): True se status = 'aguardando'
...
# ======================================================================================
# ORM: Acesso por Projeto (vínculos usuário↔projeto)
# ======================================================================================
